...
<![endif]-->
	...
	<body>
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 分类管理 <span class="c-gray en">&gt;</span> 分类列表
...
<!-- 			<div class="text-c">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" id="btndel" class="btn btn-danger radius" onclick="activity_dels(this)">
				...
				<!-- <a class="btn btn-primary radius" onclick="article_add('添加商品','/index.php/Admin/Goods/addAction/','800','500')" href="javascript:;">
					<i class="Hui-iconfont">&#xe600;</i> 添加产品</a></span> <span class="r">共有数据：<strong><?php echo $count ?></strong> 条</span> -->
			</div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<?php if(is_array($category)): foreach($category as $key=>$v): ?><tr class="text-c">
							...
							<td class="td-manage">
								<a title="编辑" href="/index.php/Admin/Category/editCategory/cat_id/<?php echo $v['cat_id'] ?>" class="ml-10" style="text-decoration:none">
									<i class="Hui-iconfont">&#xe6df;</i>
								</a>
								<a title="删除" href="javascript:;" onclick="activity_del(this,'<?php echo $v['cat_id'] ?>')" class="ml-10" style="text-decoration:none">
									<i class="Hui-iconfont">&#xe6e2;</i>
								</a>
							<!-- 	<a href="javascript:;" class="ml-10" onclick="activity_detail('查看','/index.php/Admin/Cinfo/showList/aid/<?php echo $v['id'] ?>','800','500')">参与详情</a> -->
							</td>
		...
		<script type="text/javascript">
			...
			function activity_stop(obj, id) {
				layer.confirm('确认要关闭吗？', function(index) {
					           $.ajax({
						        type: "get",
						        dataType: "json",
						        url: '/index.php/Admin/Category/endAction',
						        data: {'aid':id},
				...
				layer.confirm('确认要开启吗？', function(index) {
					//此处请求后台程序，下方是成功后的前台处理……
                           $.ajax({
						        type: "get",
						        dataType: "json",
						        url: '/index.php/Admin/Category/startAction',
						        data: {'aid':id},
						        success: function (data) {
						            if(data.code == 0) {
			...
			function activity_del(obj, id) {
				layer.confirm('确认要删除吗？', function(index) {
					$.ajax({
						type: 'get',
						url: '/index.php/Admin/Category/delCategory',
						data:{'aid':id},
						dataType: 'json',
						success: function(data) {
							if(data.code == 1){
    ...
    //给删除按钮添加点击事件(批量删除)
        ...
        window.location.href = '/index.php/Admin/Category/delAction/aid/' + ids; //Tp框架中的写法
    };
$(function(){
    //给编辑按钮添加点击事件
    $('#btnedit').on('click',function(){
        ...
        //跳转
        window.location.href = '/index.php/Admin/Category/edit/id/' + id;
    });
});
		</script>
	</body>
