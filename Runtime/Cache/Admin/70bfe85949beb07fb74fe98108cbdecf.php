           ...
           <!--   -->
            <div class="mt-20">
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <tbody>
                    <?php if(is_array($xinpin)): foreach($xinpin as $key=>$v): ?><tr class="text-c">
                            <td><?php echo $v['id'] ?></td>
                            <td><img src="<?php echo (substr($v['xinpin_logo'],1)) ?>" width="50" height="50"></td>
                            <td><?php echo $v['xinpin_name'] ?></td>
                            <td><?php echo $v['xinpin_dizhi'] ?></td>
                            <td><?php echo date('Y-m-d H:i:s', $v['add_time']) ?></td>
                            <td><?php echo date('Y-m-d H:i:s', $v['up_time']) ?></td>
                            <td class="td-manage">
                                <a title="编辑" href="/index.php/Admin/XinPin/editXinPin/id/<?php echo $v['id'] ?>" class="ml-10" style="text-decoration:none">
                                    <i class="Hui-iconfont">&#xe6df;</i>
                                </a>
                                <a title="删除" href="javascript:;" onclick="activity_del(this,'<?php echo $v['id'] ?>')" class="ml-10" style="text-decoration:none">
                                    <i class="Hui-iconfont">&#xe6e2;</i>
                                </a>
                               <a href="javascript:;" class="ml-10" onclick="activity_detail('查看','/index.php/Admin/XinPin/showList/id/<?php echo $v['id'] ?>','800','500')">查看</a>
                            </td>
        ...
        <script type="text/javascript">
            ...
            function activity_stop(obj, id) {
                layer.confirm('确认要关闭吗？', function(index) {
                               $.ajax({
                                type: "get",
                                dataType: "json",
                                url: '/index.php/Admin/XinPin/endAction',
                                data: {'aid':id},
                ...
                layer.confirm('确认要开启吗？', function(index) {
                    //此处请求后台程序，下方是成功后的前台处理……
                           $.ajax({
                                type: "get",
                                dataType: "json",
                                url: '/index.php/Admin/XinPin/startAction',
                                data: {'aid':id},
                                success: function (data) {
                                    if(data.code == 0) {
            ...
            function activity_del(obj, id) {
                layer.confirm('确认要删除吗？', function(index) {
                    $.ajax({
                        type: 'get',
                        url: '/index.php/Admin/XinPin/delXinPin',
                        data:{'aid':id},
                        dataType: 'json',
                        success: function(data) {
                            if(data.code == 1){
    ...
    //给删除按钮添加点击事件(批量删除)
        function activity_dels(obj) {
            ...
            ids = ids.substring(0,ids.length-1);
                    $.ajax({
                        type: 'get',
                        url: '/index.php/Admin/XinPin/delCategory',
                        data:{'aid':ids},
                        dataType: 'json',
                        success: function(data) {
                            if(data.code == 1){
        ...
        window.location.href = '/index.php/Admin/XinPin/delAction/aid/' + ids; //Tp框架中的写法
    };
$(function(){
    //给编辑按钮添加点击事件
    $('#btnedit').on('click',function(){
        ...
        //跳转
        window.location.href = '/index.php/Admin/XinPin/edit/id/' + id;
    });
});
        </script>
    </body>
