...
function get_goodsmenu($array,$pid=0,$num = 0){
	...
	foreach($array as $val){
		...
		if($val["parent_id"] == $pid){//判断父级id
			if($pid == 0){
				$val["sj"]="";
			}else{
				$val["sj"]=str_repeat("&nbsp;",$num)."|->";
			}
			...
			$tree = array_merge($tree,get_goodsmenu($array,$val['id'],$num + 4));
...
function get_goodsmenuid($array,$pid = 0){
	...
	foreach($array as $val){
		if($val['parent_id'] == $pid){
			//$tree .= $val["id"].",";
			$tree .=",".get_goodsmenuid($array,$val["id"]);
		}
...
/*--------------------------------------------------*/
			//获取当前分类以及子分类
/*--------------------------------------------------*/
function get_subid($array,$pid=0){
	
	$sub = get_goodsmenuid2($array,$pid);
	$sub = substr($sub,0,-1);
	return $sub;
	
}

function get_goodsmenuid2($array,$pid = 0){
	...
	foreach($array as $val){
		if($val['parent_id'] == $pid){
			//$tree .= $val["id"].",";
			$tree .=get_goodsmenuid2($array,$val["id"]);
		}
